...
prx.types.toolbar = {
	...
	,onDisplay: function(item,containerid) {
		...
		var cReturn = '<div id="'+_id+'" class="box pos type-toolbar" '+((item.overlay)? 'data-overlay="1"': '')+'><div class="inner" style="'+_bgHeader+'"></div></div>';
		return cReturn;
	}
	,properties: [
...
prx.types.tabbar = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR = cR +  '<div id="' + _id + '" class="box pos type-tabbar '+((item.iconSize==4) ? 'large-icons' : '') +'" '+((item.overlay)? 'data-overlay="1"': '')+' style="'+_bg+'">';
...
prx.types.button2 = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		text += 'text-shadow: 0px -1px 1px '+item.shadowColor+';';
		...
		var cR = "";
		cR = cR +  '<div id="' + _id + '" class="box pos type-button">';
		if(item.arrowDirection != 'none') {
		...
		case 'right':
			if(item.buttonicon.url != '') {
				cR = cR + '<img src="'+getAssetUrl(item.buttonicon)+'" style="position: absolute; '+item.iconpos+': 8px; height: '+eval((_dims.height-item.borderWidth*2)*0.2*item.iconSize)+'px; padding-top: '+eval((_dims.height-item.borderWidth*2)*0.1*(5-item.iconSize))+'px; " />'+ item.text;
			} else {
				cR = cR + item.text;
			}
			break;
		case 'notext':
			//cR = cR + '<img src="'+getAssetUrl(item.buttonicon)+'" style="height: '+eval((_dims.height-item.borderWidth*2)*0.2*item.iconSize)+'px; padding-top: '+eval((_dims.height-item.borderWidth*2)*0.1*(5-item.iconSize))+'px; " />';
...
prx.types.arrowbutton.properties = editProperty(prx.types.arrowbutton.properties, 'arrowDirection', 'values', [{value: 'left',displayValue: 'Left'},{value: 'right',displayValue: 'Right'}]);
...
prx.types.buttongroup = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		if(eval(item.bgGradient)) {
			/*
			var grad0 =  cssRGBA(item.backgroundColor, 80);
			var grad49 =  cssRGBA(item.backgroundColor, 15);
			bgBtnCss += cssGradient([{c: grad0, p: 0}, {c: grad49, p: 49}, {c: '#' + item.backgroundColor, p: 50}]);
			*/
			var _grad =  cssRGBA(item.backgroundColor, 25);
			var _grad2 =  cssRGBA(item.backgroundColor, 65);
			var _grad3 =  cssRGBA(item.backgroundColor, 15);
		...
		var cR = "";
		cR = cR +  '<div id="' + _id + '" class="box pos type-buttongroup">';
		...
		$.each(item.buttons, function(i,elm){
			...
			case 'left': 
			case 'right':
				cR = cR + '<img src="'+getAssetUrl(elm.buttonicon)+'" style="position: absolute; '+item.iconpos+': 8px; height: '+eval((_dims.height-item.borderWidth*2)*0.2*item.iconSize)+'px; padding-top: '+eval((_dims.height-item.borderWidth*2)*0.1*(5-item.iconSize))+'px; " />'+ elm.text;
				break;
			case 'notext':
...
prx.types.checkboxlist = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="label-container'+_hor+'" style="background-color: #'+item.backgroundColor+'; '+getFontCssFromFontFamily(item.textFont)+'; font-size: '+item.textSize+'px; color: #'+item.textColor+'; border-color: #'+item.borderColor+'; '+cssBorderRadius(item.borderRadius + 'px')+_props+' text-align: '+item.textAlign+';">';
...
prx.types.flipswitch = {
	...
	,onDisplay: function(item,containerid) {
		...
		cR += '<style>'
		if(!item.ios5) {
			cR += '#'+_id+' .activelabel { width: '+(item.width-2)+'px; line-height: '+(item.height-2)+'px; padding-right: '+parseInt(item.width*0.4)+'px; background-color: #'+item.activeLabelColor+'; color: #'+item.activeLabelTextColor+'; }'
			cR += '#'+_id+' .inactivelabel { width: '+(+item.width-2)+'px; line-height: '+(item.height-2)+'px; padding-left: '+parseInt(item.width*0.4)+'px; background-color: #'+item.inactiveLabelColor+'; color: #'+item.inactiveLabelTextColor+'; }'
			cR += '#'+_id+' .switch { right: '+(item.width*0.6)+'px; background-color: #'+item.switchColor+'; }'	
		} else {
			cR += '#'+_id+' label { '+cssBorderRadius(item.height + 'px')+' }'
			cR += '#'+_id+' .activelabel { width: '+(item.width-2)+'px; line-height: '+(item.height-2)+'px; padding-right: '+parseInt(item.height*0.75)+'px; background-color: #'+item.activeLabelColor+'; color: #'+item.activeLabelTextColor+'; '+cssBorderRadius([item.height/2, null, null, item.height/2])+'}'
			cR += '#'+_id+' .inactivelabel { width: '+(+item.width-2)+'px; line-height: '+(item.height-2)+'px; padding-left: '+parseInt(item.height*0.75)+'px; background-color: #'+item.inactiveLabelColor+'; color: #'+item.inactiveLabelTextColor+'; '+cssBorderRadius([null, item.height/2, item.height/2, null])+' }'
...
prx.types.picker = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		var _id = (!containerid) ? item.id : containerid+'-'+item.id;
		var cR = '<div id="' + _id + '" class="box pos type-picker"><div class="outer" style="background-color: #'+item.containerColor+';">';
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.listitems, function(i,elm){
			...
			if(elm.itemtype == 'withValue' || elm.itemtype == 'nestedWithValue' || elm.itemtype == 'withIconAndValue' || elm.itemtype == 'nestedWithBadgeAndValue') {
				...
				//_activevalue = '<span class="va-outer"><span class="va-inner" style="height: '+_height+'px; '+getFontCssFromFontFamily(item.valueFont)+_valueprops+'; font-size: '+item.valueSize+'px; color: #'+item.activeValueColor+'; padding-left: 10px;">' + elm.value + '</span></span>';
				_inactivevalue = '<span class="va-outer"><span class="va-inner value" style="height: '+_height+'px; '+getFontCssFromFontFamily(item.valueFont)+_valueprops+'; font-size: '+item.valueSize+'px; color: #'+item.valueColor+'; padding-left: 10px;">' + elm.value + '</span></span>';
			}
			/* /VALUES */
			...
			cR +='<label for="'+_id+'-checkbox-'+i+'">' +_icon + _thumbnail + _badge + _inactivevalue + _inactivetext+'</label>';
			//cR +='<label for="'+_id+'-checkbox-'+i+'" style="'+_BRstyle+'">'+_icon + _thumbnail + _badge + _activevalue + _activetext+'</label>';
			cR += '</li>';
...
removeProperties(prx.types.listnested.properties, ['valueFont', 'valueSize', 'valueColor', 'valueProperties', 'activeValueColor', 'badgeFont', 'badgeSize', 'badgeColor', 'badgeProperties', 'badgeGlassStyle', 'badgeBackgroundColor'])
removeProperties(prx.types.listnested.dynamicProperties.properties, ['itemtype', 'value', 'checked', 'badgeText']);
...
prx.types.listnested.dynamicProperties.properties = editProperty(prx.types.listnested.dynamicProperties.properties, 'itemtype', 'value', 'nested');
prx.types.listnested.dynamicProperties.properties = editProperty(prx.types.listnested.dynamicProperties.properties, 'itemtype', 'hiddenByDefault', true);
prx.types.listnested.dynamicProperties.properties = editProperty(prx.types.listnested.dynamicProperties.properties, 'buttonicon', 'hiddenByDefault', false);

//TYPE = LISTBASIC 
prx.types.listbasic = cloneobject(prx.types.listnested);
prx.types.listbasic.name = 'listbasic';
removeProperties(prx.types.listbasic.properties, ['iconpos', 'iconSize'])
removeProperties(prx.types.listbasic.dynamicProperties.properties, ['buttonicon']);
...
prx.types.listbasic.dynamicProperties.properties = editProperty(prx.types.listbasic.dynamicProperties.properties, 'itemtype', 'value', 'basic');
...
prx.types.listwithicon.dynamicProperties.properties = editProperty(prx.types.listwithicon.dynamicProperties.properties, 'itemtype', 'value', 'withIcon');
...
removeProperties(prx.types.listwithiconandvalue.properties, [/*'subFont', 'subSize', 'subColor', 'subProperties', 'activeSubColor',*/ 'badgeFont', 'badgeSize', 'badgeColor', 'badgeProperties', 'badgeGlassStyle', 'badgeBackgroundColor'])
removeProperties(prx.types.listwithiconandvalue.dynamicProperties.properties, ['itemtype', 'checked', 'badgeText']);
...
prx.types.listwithiconandvalue.dynamicProperties.properties = editProperty(prx.types.listwithiconandvalue.dynamicProperties.properties, 'itemtype', 'value', 'withIconAndValue');
prx.types.listwithiconandvalue.dynamicProperties.properties = editProperty(prx.types.listwithiconandvalue.dynamicProperties.properties, 'itemtype', 'hiddenByDefault', true);
prx.types.listwithiconandvalue.dynamicProperties.properties = editProperty(prx.types.listwithiconandvalue.dynamicProperties.properties, 'value', 'hiddenByDefault', false);
...
prx.types.listnestedwithvalue.dynamicProperties.blankItem.buttonicon = { fileId: 'a6b50021ca1cefe6af2aa49f5a703d08.png', folderId: 'f1304073334107', url: 'f1304073334107/a6b50021ca1cefe6af2aa49f5a703d08.png', assetType: 'gallery', name: ' list_arrow.png' };
prx.types.listnestedwithvalue.dynamicProperties.properties = editProperty(prx.types.listnestedwithvalue.dynamicProperties.properties, 'itemtype', 'value', 'nestedWithValue');

/* TYPE = LISTWITHVALUE */
prx.types.listwithvalue = cloneobject(prx.types.listnestedwithvalue);
prx.types.listwithvalue.name = 'listwithvalue';
removeProperties(prx.types.listwithvalue.properties, ['iconpos', 'iconSize'])
removeProperties(prx.types.listwithvalue.dynamicProperties.properties, ['buttonicon']);
...
prx.types.listwithvalue.dynamicProperties.properties = editProperty(prx.types.listwithvalue.dynamicProperties.properties, 'itemtype', 'value', 'withValue');
...
removeProperties(prx.types.listnestedwithbadge.properties, ['valueFont', 'valueSize', 'valueColor', 'valueProperties', 'activeValueColor'])
removeProperties(prx.types.listnestedwithbadge.dynamicProperties.properties, ['itemtype', 'value', 'checked']);
...
prx.types.listnestedwithbadge.dynamicProperties.blankItem.buttonicon = { fileId: 'a6b50021ca1cefe6af2aa49f5a703d08.png', folderId: 'f1304073334107', url: 'f1304073334107/a6b50021ca1cefe6af2aa49f5a703d08.png', assetType: 'gallery', name: ' list_arrow.png' };
prx.types.listnestedwithbadge.dynamicProperties.properties = editProperty(prx.types.listnestedwithbadge.dynamicProperties.properties, 'itemtype', 'value', 'nestedWithBadge');
prx.types.listnestedwithbadge.dynamicProperties.properties = editProperty(prx.types.listnestedwithbadge.dynamicProperties.properties, 'itemtype', 'hiddenByDefault', true);
prx.types.listnestedwithbadge.dynamicProperties.properties = editProperty(prx.types.listnestedwithbadge.dynamicProperties.properties, 'buttonicon', 'hiddenByDefault', false);
...
prx.types.listwithiconandbadge.dynamicProperties.properties = editProperty(prx.types.listwithiconandbadge.dynamicProperties.properties, 'itemtype', 'value', 'withIconAndBadge');


/* TYPE = LISTWITHBADGE */
prx.types.listwithbadge = cloneobject(prx.types.listnestedwithbadge);
prx.types.listwithbadge.name = 'listwithbadge';
removeProperties(prx.types.listwithbadge.properties, ['iconpos', 'iconSize'])
removeProperties(prx.types.listwithbadge.dynamicProperties.properties, ['buttonicon']);
...
prx.types.listwithbadge.dynamicProperties.properties = editProperty(prx.types.listwithbadge.dynamicProperties.properties, 'itemtype', 'value', 'withBadge');
...
prx.types.alert = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div id="' + _id + '" class="box pos type-alert"><div class="alert-inner" style="'+cssBorderRadius(item.borderRadius+'px')+' border: '+item.borderWidth+'px solid #'+item.borderColor+'; background: '+_bg+';background-color: #' + item.backgroundColor + ';"><span class="alert-title">' + item.title + '</span><span class="alert-text">' + item.text + '</span>';
...
prx.types.actionsheet = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		var cR = '';
		cR += '<div id="' + _id + '" class="box pos type-actionsheet"><div style="padding: '+item.padding+'px; '+_bg+' background-size: 100% 20px; background-repeat: no-repeat; background-color: #'+item.backgroundColor+';">';
		$.each(item.buttons, function(i,elm){
...
prx.types.popover = {
	name: "popover"
		...
		if(eval(item.hasHeader)) {
			...
			cR += '<div style="line-height: 30px; height: 25px; '+_bgHeader+' padding: 5px 0; text-align: center;'+getFontCssFromFontFamily(item.textFont)+'; color: #'+item.textColor+'; font-size: '+item.textSize+'px; ' + cssBorderRadius(item.borderRadius+'px '+item.borderRadius+'px 0 0') + '">'+item.text+'</div>';			
		}
		cR += '<div class="popover-content" style="height: '+_height2+'; '+_borderRadius+' border: 5px solid #'+item.borderColor+'; background-color: #'+item.backgroundColor+'"></div>';
		cR += '</div></div>';
...
prx.types.propertiestest = {
	name: "propertiestest"
	,onDisplay: function(item,containerid,pageid,symbol) {
		var _id = (!containerid) ? item.id : containerid+'-'+item.id;
		
		var cR = '<div id="' + _id + '" class="box pos type-tabbar">';
...
prx.components.propertiestest = {
	name: 'propertiestest'
	,type: 'propertiestest'
	,lib: _library
	,caption: 'Properties test'
	,icon: '-160px -400px'
	,helper: prx.url.devices+_path + 'alert/helper.png'
	,width:"250"
	,height:"150"
